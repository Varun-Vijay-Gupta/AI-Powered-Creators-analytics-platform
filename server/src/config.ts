...
export const config = {
  ...
  embeddingModel:
    process.env.EMBEDDING_MODEL ?? "bge-small-en-v1.5",

  embeddingDimensions: 384,

  embeddingCacheDir:
    process.env.EMBEDDING_CACHE_DIR ?? "./.embedding-cache",
  ...
  whisperLocalModel:
    process.env.WHISPER_LOCAL_MODEL ?? "tiny",

  whisperPath:
    process.env.WHISPER_PATH?.trim() ?? "",
